...
2. Log in with the **API bearer token**: it is auto-generated by the template and stored in the `API_BEARER_TOKEN` environment variable (Notploy → your service → Environment). The same token authenticates REST API requests (`Authorization: Bearer <token>`).
