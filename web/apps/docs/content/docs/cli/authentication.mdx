...
description: A guide to authenticating with the Notploy CLI
---

The Notploy CLI uses a token-based authentication system. To authenticate, you'll need to create an access token and store it securely.
...
Notploy when you create an access token automatically will generate a config.json with the access token and the server url.



## Commands

1. `notploy auth -u <host> -t <token>` - Authenticate with the Notploy CLI.
2. `notploy verify` - Verify if the access token is valid.
