...
Telegram notifications are a great way to stay up to date with important events in your Notploy panel. You can choose to receive notifications for specific events or all events.
...
3. Set a name for your bot, eg. `notploy_bot` make sure the name ends with `_bot`.
4. Copy the `Bot Token` and paste it in Notploy `Telegram` Modal section.
...
8. Copy the `Chat ID` and paste it in Notploy `Telegram` Modal section.
