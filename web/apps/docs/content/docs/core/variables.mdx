...
description: "Notploy allows you to create and manage shared and service-level environment variables for your projects and environments."
---
...
Environment variables in Notploy allow you to:
...
<Callout type="info">
  Notploy will automatically replace `${{project.DATABASE_URL}}` with the value defined in the project's shared variables.
</Callout>

You can use shared environment variables in all the services available in notploy.
...
<Callout type="info">
  Notploy will automatically replace `${{environment.VARIABLE_NAME}}` with the value defined in the environment's variables.
</Callout>
...
<Callout type="info">
  Preview Deployments environments also include a service-level variable called `NOTPLOY_DEPLOY_URL`, which points to the deployment URL of the service.
  It can be used as `${{NOTPLOY_DEPLOY_URL}}` for variables like `APP_URL=https://${{NOTPLOY_DEPLOY_URL}}`.
</Callout>
...
Values are fetched at deploy time and never stored in Notploy. See [Secrets Providers](/docs/core/secrets-providers) for setup, reference formats and access control.
