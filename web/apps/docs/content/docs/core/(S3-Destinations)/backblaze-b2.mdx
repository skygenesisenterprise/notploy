...
- `Access Key` -> `Access Key (Notploy)` = eg. `002s6acf2639910000d000005`
- `Secret Key` -> `Secret Key (Notploy)` = eg. `K00+rIsWqPMhmcgqcyOyb9bqby7pbpE`
- `Region` -> `Region (Notploy)` = eg. `eu-central-003, us-east-005, us-west-002, us-west-001, us-west-004, etc` it will depend on the region you are using.
- `Endpoint` -> `Endpoint (Notploy)` = eg. `https://s3.us-west-002.backblazeb2.com` you will find this endpoint in the Bucket Card at the Home Page.
- `Bucket` -> `Bucket (Notploy)` = eg. `notploy-backups` use the name of the bucket you created.
