...
Notploy offers a UI to connect to any Docker Registry.
...
- **Image Prefix(Optional)**: Useful when using Cluster feature, to tag your images with a prefix eg. `notploy` will convert to `notploy/my-app:latest`.
...
Notploy will default to using its own server.