...
3. Give it a name (e.g. `notploy`) and click **Next**.
...
6. Add the following **Redirect URI**:
   - `https://your-notploy-domain.com/api/auth/sso/callback/{providerId}`
   Replace `{providerId}` with the unique identifier you will use in Notploy (e.g. `zitadel`).
...
Notploy will automatically fetch the OpenID Connect discovery document from `{issuer}/.well-known/openid-configuration`.

## 3. Configure Notploy

1. In Notploy, go to **Settings** → **SSO** → **Register OIDC Provider**.
2. Enter:
...
3. Note the **Callback URL** shown by Notploy (e.g. `https://your-notploy-domain.com/api/auth/sso/callback/zitadel`).
4. Save.

## 4. Verify the Redirect URI in Zitadel

Make sure the Redirect URI configured in your Zitadel application matches exactly the Callback URL shown in Notploy:

```
https://your-notploy-domain.com/api/auth/sso/callback/zitadel
```
...
- **Redirect URI mismatch** — The Redirect URI in Zitadel must match the Callback URL in Notploy exactly, including the `{providerId}` path segment.
