...
5. Set **Root URL** to your Notploy base URL, e.g. `https://your-notploy-domain.com`.
6. Save.
...
3. Enter:
   ...
   - **Domain**: the domain users use to authenticate via Keycloak (e.g. your organization domain like `acme.com`), not the Notploy instance URL
...
2. Set **Valid redirect URIs** to your Notploy callback URL, for example:
   - `https://your-notploy-domain.com/api/auth/callback/my-client-id`
...
- **Redirect URI mismatch** — Ensure the callback URL in Notploy matches exactly what is configured in Keycloak (including protocol and path). Use the same **Provider** value in the path (e.g. `.../api/auth/callback/myorg-name-keycloak`).
...
- **Attribute mapping** — If user email or name is missing, map Keycloak attributes (e.g. email, preferred_username) in Notploy if your setup supports it.
