...
Remote servers allows you to deploy your apps remotely to different servers without needing to build and run them where the Notploy UI is installed.

## Requirements

1. To install Notploy UI, follow the [installation guide](en/docs/core/get-started/installation).

<Callout type="warning">
  If your remote server is configured with a different shell (other than bash), you must configure bash as the default shell, as Notploy has been developed and tested with bash.
</Callout>
...
- [Hostinger](https://www.hostinger.com/vps-hosting?ref=notploy) Get 20% off with this [referral link](https://www.hostinger.com/vps-hosting?REFERRALCODE=1SIUMAURICI97).
...
7. Click `Setup Server` to proceed. There are two tabs: SSH Keys and Deployments. This guide explains the easy way, but you can follow the manual process via the Notploy UI if you prefer.
...
<Callout>
  You only need to run this setup once. If Notploy updates later, check the
...
10. To check which server an app belongs to, you’ll see the server name at the top. If no server is selected, it defaults to `Notploy Server`. Click `Deploy` to start building your app on the remote server. You can check the `Logs` tab to see the build process. For this example, we’ll use a test repo:  
    Repo: `https://github.com/skygenesisenterprise/notploy.git`  
    Branch: `main`  
    Build Path: `/astro`
