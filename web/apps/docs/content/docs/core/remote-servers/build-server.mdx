...
A **Build Server** is a remote server dedicated to building and compiling your applications. Instead of building your application on the deployment server, Notploy will:
...
1. A Notploy instance running
2. A remote server for builds (VPS, cloud instance, or dedicated server)
3. SSH access to the build server
4. Docker installed on the build server (or use Notploy's automatic setup)
...
Navigate to **Dashboard → Remote Servers → Add Server -> Build Type (Build Server)** in your Notploy dashboard.
...
1. **Build Phase**:
   - Notploy connects to your build server via SSH
...
2. **Deploy Phase**:
   - Notploy connects to your deployment server(s)
