...
description: Learn how to deploy your application in production in Notploy.
---

import { Callout } from 'fumadocs-ui/components/callout';

By default, notploy offer multiple [Builds Types](/docs/core/applications/build-type) to deploy your application, the most common is `nixpacks` and `heroku buildpacks`
...
1. **Use Git Provider in Your Application**:
   - Repository: `https://github.com/Notploy/production-example`
   - Branch: `main`
   - Build path: `/`
...
9. Now let's create application in Notploy.
  ...
  build-and-push-dockerfile-image:
    ...
    steps:
      ...
      - name: Trigger Notploy Deployment
        uses: notploy/notploy-action@v1
        run: |
            curl -X 'POST' \
            'https://<your-notploy-domain>/api/application.deploy' \
...
You can also use this Github Action [Action](https://github.com/marketplace/actions/notploy-deployment) to automate the deployment.

## Healthcheck & Rollbacks

When using Notploy you can also configure healthchecks and rollbacks, this will allow you to configure your application to be able to recover from failures.
