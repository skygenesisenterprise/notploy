...
This section is designed for experienced users who need to manage complex configurations and orchestration settings in Notploy. The **Advanced** tab is located in your **Applications** settings. Here, you can execute custom commands, manage cluster replicas, select Docker registries, and configure Docker Swarm settings.
...
1. **Build Phase**: Notploy connects to your build server via SSH, clones your repository, and builds the Docker image on the build server
...
<Callout type="warn">
  **Required Configuration**: When using a build server, you must configure a Docker registry in your Notploy settings. The built image needs to be stored in a registry that's accessible to your deployment servers. See [Docker Registry](/docs/core/registry) for configuration details.
</Callout>
...
Docker API expects memory values in **bytes**. Notploy passes the value you enter directly to the Docker API, so you must enter the value in raw bytes. Units like `MB` or `GB` are not parsed.
...
Docker API expects CPU values in **nanoCPUs**, where `1000000000` nanoCPUs equal 1 CPU core. Notploy passes the value you enter directly to the Docker API, so you must enter the value in raw nanoCPUs. Decimal core values like `0.5` are not parsed.
...
When Notploy communicates with Docker API, these values are sent in the service specification:
...
File mounts are a Notploy feature. When you create a file mount, Notploy stores the file in a folder called `files` inside your project directory. This file is created once when you set up the file mount and persists across deployments.
