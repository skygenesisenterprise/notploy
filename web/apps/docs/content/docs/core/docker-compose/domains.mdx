...
## Method 1: Using Notploy Domains (Recommended)

The easiest way to configure domains is using Notploy's native domain management feature. This method allows you to configure domains directly through the Notploy UI without manually editing your Docker Compose file.

<Callout type="info">
  **Recommended**: Since v0.7.0, Notploy supports domains natively. You can configure your domains directly in the Notploy UI through the **Domains** tab of your Docker Compose application. This is the simplest and most user-friendly approach.
</Callout>

To use this method:
1. Navigate to your Docker Compose application in Notploy
2. Go to the **Domains** tab
3. Click **Add Domain** and configure your domain
4. Notploy will automatically handle the routing configuration

### How It Works

At runtime, during the deployment phase, Notploy automatically adds Traefik labels internally to your Docker Compose file. You don't need to manually add these labels - Notploy handles this automatically based on the domains you configure in the UI.
...
- If you're not using [Isolated Deployments](/docs/core/docker-compose/utilities#isolated-deployments), Notploy will add the `notploy-network` to the service you selected, however you need to add `notploy-network` to the other services to maintain connectivity.
- The Preview Compose button is useful for verifying how Notploy will modify your compose file before deployment.
- All label generation and network configuration is handled automatically by Notploy based on your domain settings.

For detailed instructions on using Notploy's domain management, see the [Domains guide](/docs/core/domains).
...
<Callout type="warn">
  Manual configuration is more complex and requires editing your Docker Compose file. We recommend using Method 1 (Notploy Domains) unless you have specific requirements that need manual Traefik label configuration.
</Callout>
...
1. Add the service to the `notploy-network`.
...
First, we'll add the notploy-network to our services:

<Callout type="info">
  **Tip**: If you prefer to isolate all services and avoid adding them to the `notploy-network`, you can use the **Isolated Deployments** feature. This feature isolates all services and eliminates the need to manually add them to the notploy-network. See the [Isolated Deployments guide](/docs/core/docker-compose/utilities#isolated-deployments) for more information.
</Callout>
...
services:
  frontend:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.frontend-app.rule=Host(`frontend.notploy.com`)
  ...
  backend:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.backend-app.rule=Host(`backend.notploy.com`)
...
2. `traefik.http.routers.<UNIQUE-RULE>.rule=Host('your-domain.notploy.com')` Specifies the domain for the service
...
4. **Isolated Deployments**: As an alternative to manually adding services to `notploy-network`, you can use the [Isolated Deployments](/docs/core/docker-compose/utilities#isolated-deployments) feature, which automatically isolates all services and handles networking configuration for you.
...
<Callout type="info">
  Remember: For most use cases, we recommend using **Method 1 (Notploy Domains)** as it's simpler and doesn't require manual Docker Compose file editing. See the [Domains guide](/docs/core/domains) for more information.
</Callout>
