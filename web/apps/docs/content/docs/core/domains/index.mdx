---
title: Domains
description: This section outlines how to configure domains for your applications in Notploy, ensuring that your applications are accessible via custom URLs.
---

import { Callout } from 'fumadocs-ui/components/callout';
import { Link } from 'next/link';

Notploy Provide 2 ways to add a domain to your service:
...
If you don't have a domain, you can use our integrated free domains from Traefik.me in the Notploy panel, without doing any configuration however is only for HTTP, if you want to have a HTTPS domain you need to buy a domain from one of the providers above.
...
Notploy handles domains differently depending on whether you're using Applications or Docker Compose:

### Applications

For **Applications**, Notploy creates a unique configuration file for each domain you create or update. These files are managed automatically and can be viewed in:

- **Traefik File System**: Accessible through the Notploy UI
...
http:
  routers:
    notploy-app-name-router-1:
      rule: Host(`your-domain.com`)
      service: notploy-app-name-service-1
      middlewares:
        - redirect-to-https
      entryPoints:
        - web
  ...
  services:
    notploy-app-name-service-1:
      loadBalancer:
        servers:
          - url: http://notploy-app-name:3000
        passHostHeader: true
```
...
- **Hot Reload**: Notploy uses Traefik's [File Provider](https://doc.traefik.io/traefik/reference/install-configuration/providers/others/file/) which supports hot reloading, so configuration changes are applied automatically
...
- **Host**: The domain name that you want to link to your application (e.g., `api.notploy.com`).
...
Notploy uses Traefik middlewares to modify request paths before they reach your application. These powerful tools allow you to create flexible routing configurations that match your application's expected URL structure.
...
**Example:**
- Domain: `api.notploy.com`
- Path: `/v1`
- Internal Path: `/backend/api`
- Request: `api.notploy.com/v1/users`
...
**Example:**
- Domain: `app.notploy.com`
- Path: `/dashboard`
- Strip Path: Enabled
- Request: `app.notploy.com/dashboard/settings`
...
**Example:**
- Domain: `service.notploy.com`
- Path: `/public`
- Strip Path: Enabled
- Internal Path: `/app/v2`
- Request: `service.notploy.com/public/api/users`
- Processing:
...
Create the domain in notploy, using the host `www.example.com`

Now in notploy, you can go to application -> advanced -> redirects (Select the preset www to non-www) and click on save.
