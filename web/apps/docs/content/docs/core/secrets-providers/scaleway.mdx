...
description: "Use Scaleway Secret Manager secrets in your Notploy environment variables."
---
...
Notploy authenticates with a Scaleway **API key** scoped to the project your secrets live in.
...
<Callout type="warn">
  The Secret Key is the credential, there is no separate access key ID — Notploy authenticates with it alone via the `X-Auth-Token` header.
</Callout>
