	...
	useEffect(() => {
		const fetchStats = async () => {
			const [starsRes, dockerRes, contribRes] = await Promise.allSettled([
				fetch("/api/github-stars?owner=notploy&repo=notploy"),
		...
		<div className="flex flex-col gap-10 px-4 py-20 lg:py-40 ">
			<div className="mx-auto max-w-2xl md:text-center">
				...
				<p className="mt-4 text-center text-lg tracking-tight text-muted-foreground">
					...
					Turns out, Notploy has actually helped a few people—who knew?
				</p>
			</div>
...
function getGrid({
	...
	return [
		{
			title: "GitHub Stars",
			description: `With over ${(githubStars / 1000).toFixed(1)}k stars on GitHub, Notploy is trusted by developers worldwide. Explore our repositories and join our community!`,
			icon: (
		...
		},
		{
			...
			description: `Downloaded over ${Math.floor(dockerDownloads / 1000000)}M times, Notploy has become a go-to solution for seamless deployments. Discover our presence on DockerHub.`,
			icon: (
				<svg
					stroke="currentColor"
					fill="currentColor"
		...
		},
		{
			...
			description: `Thanks to our growing base of over ${contributors} contributors, Notploy continues to thrive, with valuable contributions from developers around the world.`,
		...
		{
			title: "Sponsors",
			description: `More than ${defaultStats.sponsors} companies/individuals have sponsored Notploy, ensuring a steady flow of support and resources. Join our community!`,
