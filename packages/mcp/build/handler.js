        ...
        try {
            ...
            if (error instanceof Error) {
                if (error.message.includes("401") || error.message.includes("Unauthorized")) {
                    return ResponseFormatter.error(`Authentication failed for ${tool.name}`, "Please check your NOTPLOY_API_KEY configuration");
                }
                if (error.message.includes("404") || error.message.includes("Not Found")) {
                    return ResponseFormatter.error("Resource not found", `The requested resource for ${tool.name} could not be found`);
                }
            }
            return ResponseFormatter.error(`Failed to execute ${tool.name}`, `Error: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    };
}
