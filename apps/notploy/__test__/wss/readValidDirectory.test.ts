...
vi.mock("@notploy/server/constants", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("@notploy/server/constants")>();
	return {
		...actual,
		paths: () => ({
			...actual.paths(),
...
const { readValidDirectory } = await import("@notploy/server");
