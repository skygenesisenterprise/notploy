...
interface Props {
	/** Selected server; undefined shows the local Notploy server */
	serverId?: string;
}
				...
				cell: ({ row }) => (
					<div className="flex items-center justify-end gap-3">
						...
						<DialogAction
							...
							description={`The network "${row.original.name}" will be removed from Docker and Notploy. This action cannot be undone.`}
							onClick={async () => {
								try {
									await removeNetwork({
										networkId: row.original.networkId,
