		...
		form.reset({
			services: services.map((serviceName) => {
				...
				return {
					...
					detachNotployNetwork: config?.detachNotployNetwork ?? false,
				};
			}),
		});
	...
	const onSubmit = async (values: FormValues) => {
		try {
			const serviceNetworks = values.services.filter(
				(s) => s.networkIds.length > 0 || s.detachNotployNetwork,
			);
			await mutateAsync({
				composeId,
				serviceNetworks,
			...
			<CardHeader className="flex flex-row items-start justify-between flex-wrap gap-2">
				<div className="flex flex-col gap-1">
					...
					<CardDescription>
						...
						notploy-network. Takes effect on the next deploy.
					</CardDescription>
				</div>
				<Button
					variant="outline"
